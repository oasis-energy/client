...
export function EcoModernCard({ side, className, id, data }: VisitingCardProps) {
  if (side === "front") {
    return (
    ...
    <div id={id} className={`w-[1050px] h-[600px] bg-[#fdfdfc] text-slate-900 relative overflow-hidden flex shadow-md border border-slate-100 rounded-[2rem] ${className || ""}`}>
      ...
      <div className="flex-1 flex px-24 pt-20 pb-20 z-10 w-full">
        ...
        <div className="flex flex-col justify-between w-2/3 pr-16">
          <div>
            <h1 className="text-6xl font-bold text-slate-900 mb-3 tracking-tight font-heading">{data.name.split(" ").length > 1 ? <>{data.name.split(" ")[0]}<br/>{data.name.split(" ").slice(1).join(" ")}</> : data.name}</h1>
            <h2 className="text-xl font-semibold text-emerald-600 tracking-widest uppercase">{data.role}</h2>
          </div>
          
          <div className="flex flex-col gap-6 mt-8">
            <div className="flex items-center gap-5 text-slate-600 border-b border-slate-100 pb-4">
              <Phone className="w-5 h-5 text-emerald-500" />
              <span className="text-xl font-medium">{data.phone}</span>
            </div>
            ...
            <div className="flex items-center gap-5 text-slate-600 border-b border-slate-100 pb-4">
              ...
              <span className="text-xl font-medium">{data.website}</span>
            </div>
            ...
            <div className="flex items-center gap-5 pt-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-lg font-bold text-slate-700 bg-slate-100 px-3 py-1 rounded-full">{data.socialHandle}</span>
              </div>
            </div>
          </div>
        </div>
